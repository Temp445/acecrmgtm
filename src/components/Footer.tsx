    ...
    <div className="bg-gray-900 text-white mt-10 lg:mt-0 py-8 lg:py-0 bottom-0">
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16">
        ...
        <div className="flex items-center gap-2 mb-8 pt-6 ">
          ...
          <h2 className="text-sm md:text-base xl:text-xl font-semibold">ACE CMS</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 mb-8 px-5">

          <div className="flex flex-col space-y-4">
            <h2 className="text-sm md:text-base xl:text-lg font-semibold">{t('contactUs')}</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col space-y-4">
              <h2 className="text-sm md:text-base xl:text-lg font-semibold">{t('links.QuickLinks')} </h2>
              <div className="flex flex-col space-y-2 md:space-y-3">
                <Link href="/#features" className="text-xs md:text-sm xl:text-base">{t('links.Features')}</Link>
                <Link href="/#pricing" className="text-xs md:text-sm xl:text-base">{t('links.Pricing')}</Link>
                <Link href="/#client" className="text-xs md:text-sm xl:text-base ">{t('links.Clients')}</Link>
                <Link href="/#contact" className="text-xs md:text-sm xl:text-base ">{t('links.Contact')}</Link>
              </div>
            </div>

            <div className="flex flex-col space-y-4  ">
              <h2 className="text-sm md:text-base xl:text-lg font-semibold">{t('links.help')}</h2>
              <div className="flex flex-col space-y-2 md:space-y-2">
                ...
                <Link href="/book-demo" className="text-xs md:text-sm xl:text-base hover:text-gray-300 transition-colors ">{t('links.bookDemo')}</Link>
                <Link href="https://acesoft.in/privacy" className="text-xs md:text-sm xl:text-base hover:text-gray-300 transition-colors ">{t('links.privacy')}</Link>
                <Link href="https://acesoft.in/terms" className="text-xs md:text-sm xl:text-base hover:text-gray-300 transition-colors ">{t('links.terms')}</Link>
              </div>
            </div>
          </div>
        ...
        <p className="text-xs md:text-sm text-center text-gray-300 pb-4">
